...
actor1_config = ezros_tools.package_path() + "/cfg/actor1.yaml"
actor1 = EzRobot("actor1", actor1_config)  # Initialize the EzRobot using the config file
...
actor1.drive_for(speed=1.0, duration=1.0)  # Drive at speed 1.0m/s for 1 seconds
actor1.stop()  # Stop the robot instantly with the built-in stop function
...
def stop_at_crosswalk(target_percent: int = 50) -> bool:
    ...
    return True if actor1.msg_white_percent.data > target_percent else False


# Now we can use the function to drive the robot until the crosswalk is detected
actor1.print_highlights("Driving until crosswalk is detected...")  # Print a "checkpoint" message to help with debugging
actor1.drive_for(speed=0.15, end_function=stop_at_crosswalk, end_function_kwargs={"target_percent": 40})
...
actor1.print_highlights("Stopping until crosswalk is clear...")  # Print a "checkpoint" message to help with debugging
...
def crosswalk_is_clear(wait_for=10.0) -> bool:
    ...
    if time.time() - initial_time > wait_for:
        actor1.print_highlights("Crosswalk is clear!")
        return True
    else:
        return False
...
actor1.stop(duration=crosswalk_is_clear)
actor1.drive_for(speed=0.5, speed_distance=1.0)  # Start driving again for X "speed derived" meters
actor1.stop(duration=3.0)  # Another method of stopping the robot by using the built-in time duration function.
